...
PAYMENT_STATUS_UNPAID = 0          # Не оплачено магазином (Курьер: Жду оплаты)
PAYMENT_STATUS_MARKED_PAID = 1     # Оплачено магазином (Курьер: Подтвердить оплату)
PAYMENT_STATUS_CONFIRMED = 2       # Курьер подтвердил (Успешно)

# Commands for roles
SHOP_COMMANDS = [
    BotCommand("new_order", "Создать заказ"),
    BotCommand("myorders", "Активные заказы"),
    BotCommand("unpaid", "Неоплаченные заказы"), # НОВАЯ КОМАНДА
...
async def init_db(app):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            """)
            # orders: main table
            # PAID_TO_COURIER теперь INTEGER (0, 1, 2)
            await db.execute("""
...
async def save_order(data: dict):
    ...
    # Определяем статус: если это возврат, статус будет 'taken' (сразу у курьера), иначе 'new'
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """, (
            data["shop_tg_id"],
            data.get("courier_tg_id"), # Для возврата курьер уже назначен
            data["from_address"],
            data["shop_contact"],
            data["to_address"],
            data.get("to_apt", ""), # Оставляем в БД, но убираем из FSM
            ...
            PAYMENT_STATUS_UNPAID, # По умолчанию 0
        ))
        await db.commit()
        return cur.lastrowid

# Вспомогательная функция для частичной оплаты (70%)
async def pay_courier_fixed_amount(order_id: int, courier_id: int, price: float, log_message: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # Обновляем заказ на оплаченный
        # В этой функции МЫ НЕ УСТАНАВЛИВАЕМ PAID_TO_COURIER = 1 (MARKED_PAID) или 2 (CONFIRMED)
        # Мы только регистрируем оплату в логе и таблице payments
...
async def get_order(order_id: int):
    ...
    return {
        ...
        "created_at": row[12],
        "return_for": row[13],
        "paid_to_courier": row[14], # 0, 1, или 2
        "paid_at": row[15],
    }
...
async def update_order(order_id: int, status: Optional[str] = None, courier: Optional[int] = None, log_add: Optional[str] = None, paid: Optional[int] = None):
    ...
    paid (0, 1, 2) -> set paid_to_courier
    """
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(DB_PATH) as db:
        if log_add:
...
async def purge_chat_history(app, chat_id: int, ctx: ContextTypes.DEFAULT_TYPE):
    """Удаляет все сообщения, отправленные ботом, ID которых сохранены."""
    
    # Инициализация хранилища, если оно отсутствует
    ...
    for mid in messages_to_delete:
        if mid:
            try:
                # В Telegram бот может удалить только свои сообщения, отправленные 
                # в течение 48 часов, или любые, если он админ.
...
async def register_bot_message(ctx: ContextTypes.DEFAULT_TYPE, message_id: int):
    """Сохраняет ID нового сообщения бота для последующей очистки."""
    if "all_bot_messages" not in ctx.user_data:
        ctx.user_data["all_bot_messages"] = []
...
async def delete_message_after_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Удаляет только сообщение с командой пользователя."""
    if update.message:
        try:
...
def html_report(order_id: int, o: dict, courier_name: Optional[str], include_log: bool = True):
    ...
    # Определение заголовка в зависимости от типа заказа
    header = f"<b>ЗАКАЗ #{order_id}</b>"
    if o.get("return_for"):
        # Визуальная связь заказа в формате X.Y
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    reply_msg = await update.message.reply_text("Кто вы?", reply_markup=InlineKeyboardMarkup(kb))
    await register_bot_message(ctx, reply_msg.message_id)
...
async def role_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # После выбора роли удаляем меню выбора
    try:
        await q.edit_message_reply_markup(reply_markup=None)
        # Удаляем сообщение из истории, так как оно деактивировано
...
# ========== WHOAMI / STATS (Тотальная очистка) ==========
...
async def new_order_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["return_for_id"] = None # Сброс флага возврата
...
async def shop_set_return_order_entry(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Ошибка парсинга create_return_from_fail_: {e}, data: {q.data}")
        return ConversationHandler.END
    ...
    if not original_order["courier_tg_id"]:
        await q.edit_message_text(f"⛔ Для заказа #{order_id} не назначен курьер (заказ не был взят).")
        return ConversationHandler.END
        
    # Двойная проверка: если уже есть незавершенный возвратный заказ
    async with aiosqlite.connect(DB_PATH) as db:
    ...
    if count_returns > 0:
        await q.edit_message_text(f"⛔ Для заказа #{order_id} уже создан или находится в работе возвратный заказ. Создание дубликата невозможно.")
        return ConversationHandler.END
    ...
    ctx.user_data["courier_tg_id"] = original_order["courier_tg_id"] # Присваиваем курьера из оригинального заказа

    # --- АВТОЗАПОЛНЕНИЕ ДЛЯ ВОЗВРАТНОГО ЗАКАЗА ---
    # Заполняем все поля автоматически, инвертируя from/to
    ctx.user_data["from_address"] = original_order["to_address"] # От клиента
    ctx.user_data["shop_contact"] = original_order["shop_contact"] # Контакт магазина
    ctx.user_data["to_address"] = original_order["from_address"] # К магазину
    ...
    # 🔔 ИСПРАВЛЕНИЕ 1: Отмечаем исходный заказ как completed_with_return, чтобы он не "висел"
    await update_order(
        order_id, 
    ...
    # Сразу переходим к последнему шагу FSM для ввода цены
...
async def edit_order_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Запрет на редактирование, если на финальной стадии или завершен
...
async def step_from(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Если это возвратный заказ, пропускаем до цены
...
async def step_client_address(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["to_apt"] = "" # Явно сбрасываем кв/офис, так как оно не запрашивается
...
async def step_price_final(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if order_id is None:
        ...
        if is_return:
             # Если это возвратный заказ, назначаем его курьеру и уведомляем
             ...
             # Отправка нового возвратного заказа курьеру
             try:
                ...
                logger.warning(f"Не удалось отправить уведомление о новом возвратном заказе курьеру: {e}")
        ...
        courier_id_updated = new_order_data.get("courier_tg_id") # Используем обновленный ID
        ...
        if courier_id_updated:
            ...
            try:
                ...
                if getattr(sent_msg, "message_id", None):
                    # Деактивируем все старые сообщения и сохраняем новое
            ...
            # Если заказ вернулся в new, рассылаем всем
            await send_order_to_couriers(oid, ctx.application)
...
async def myorders(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Активные статусы (ВСЕ, кроме полностью завершенных, оплаченных или требующих внимания)
    ...
    for row in rows:
        ...
        # --- Кнопки для Магазина (Активные заказы) ---
        if role == "shop":
            kb = []
        ...
        # --- Кнопки для Курьера (Активные заказы) ---
        else:
            if is_return_order:
                if shop_tg_id:
        ...
        # Отправляем каждое сообщение отдельно для кнопок
        if final_kb:
...
# ========== UNPAID_ORDERS (Неоплаченные заказы) - НОВАЯ КОМАНДА ==========
async def unpaid_orders(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        if role == "shop":
            # Магазин видит все, где оплата не подтверждена (0 или 1)
            ...
            # Курьер видит все, где оплата не подтверждена (0 или 1)
    ...
    report_title = f"💸 <b>НЕОПЛАЧЕННЫЕ ЗАКАЗЫ ({role.upper()})</b>"
    ...
    for row in rows:
        ...
        if role == "shop":
            if paid_to_courier == PAYMENT_STATUS_UNPAID:
                ...
                report += "УСПЕШНО"
                kb.append([InlineKeyboardButton("✅ Успешно", callback_data=f"info_success")])
...
# ========== ОБРАБОТЧИКИ ОПЛАТЫ (Новая логика) ==========

# Магазин отмечает как оплачено
async def shop_mark_paid_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. Обновляем сообщение у магазина
    ...
    # 3. Уведомляем курьера о необходимости подтверждения
    ...
    try:
        await ctx.application.bot.send_message(
            courier_id,
            f"🔔 <b>ПОСТУПИЛА ОПЛАТА!</b>\nМагазин #{shop_id} ({await get_username(shop_id, ctx.application)}) отметил заказ #{order_id} как оплаченный. Подтвердите получение.",
            parse_mode="HTML",
...
async def courier_confirm_payment_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Сумма к оплате (с учетом 70% отмены)
    amount = float(o['price'] or 0)
    if o['status'] == 'cancelled_70_percent':
         amount = round(amount * 0.70, 2)
         
    # 1. Обновляем статус: 1 -> 2 (Успешно)
    ...
    # 2. Добавляем запись в таблицу payments (это происходит только один раз при подтверждении)
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT INTO payments (order_id, shop_tg_id, courier_tg_id, amount, paid_at) VALUES (?, ?, ?, ?, ?)",
    ...
    # 3. Обновляем сообщение у курьера
    ...
    # 4. Уведомляем магазин об успехе
...
# ========== COMPLETED_ORDERS (Завершенные заказы) - Тотальная очистка ==========
...
async def send_completed_for_role(tg: int, role: str, ctx: ContextTypes.DEFAULT_TYPE):
    # Добавляем completed_with_return в завершенные
...
async def history_switch_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Редактируем кнопку, чтобы она исчезла (вместо удаления всего сообщения)
    try:
         await q.edit_message_reply_markup(reply_markup=None)
    except:
         pass
...
# ========== COURIER ACTIONS (без изменений, кроме регистрации сообщений) ==========
...
async def picked_up(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not o or o["courier_tg_id"] != courier_id or o["status"] != 'at_shop':
        await q.answer("⛔ Заказ неактивен или вы не в магазине.")
        return

    courier_name = await get_username(courier_id, ctx.application)
    shop_id = o["shop_tg_id"]

    await update_order(order_id, status="on_delivery", log_add=f"Курьер {courier_name} забрал товар и направляется к клиенту.")

    # Обновление кнопок и отчета у курьера
    o = await get_order(order_id)
    ...
    try:
        ...
        await register_bot_message(ctx, sent_msg.message_id) # Сохраняем ID нового сообщения
...
async def take_order(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    courier_name = await get_username(courier_id, ctx.application)
    # Статус меняется на 'taken' (в пути к магазину)
    await update_order(
        order_id,
        status="taken",
        courier=courier_id,
    ...
    o = await get_order(order_id)
    txt = html_report(order_id, o, courier_name, include_log=True) # Полный отчет
    
    # Кнопки для taken: Написать магазину, У магазина, Отменить
    kb = [
    ...
    try:
        # Редактируем сообщение, которое курьер нажал, чтобы взять заказ
        await q.edit_message_text(f"🚀 Вы взяли заказ #{order_id}. Направляйтесь к магазину.", reply_markup=None)
    except:
        pass
        
    # Отправляем курьеру новое активное сообщение
    try:
        sent_msg = await ctx.application.bot.send_message(
            courier_id,
            txt,
...
async def arrived(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    status_map = {
        "shop": ("at_shop", "в магазин"),
        "client": ("at_client", "получателю")
    }
    
    if where == "shop" and o["status"] != 'taken':
        await q.answer("⛔ Вы еще не взяли заказ или уже забрали товар.")
        return
    elif where == "client" and o["status"] != 'on_delivery':
        await q.answer("⛔ Вы еще не забрали товар у магазина.")
        return
    ...
    log_msg = f"Курьер прибыл {location_name}" # Упрощенное сообщение для лога
    await update_order(order_id, status=new_status, log_add=log_msg)
    
    o = await get_order(order_id)
    report = html_report(order_id, o, courier_name, include_log=True)
    
    # Формирование кнопок в зависимости от места прибытия
    kb = []
    if where == "shop":
        # Кнопки: Забрал, Отменить
        kb = [
        ...
        # Кнопки: Доставлен, Клиент отсутствует
        kb = [
            [InlineKeyboardButton("✅ Доставлен", callback_data=f"finish_{order_id}")],
            [InlineKeyboardButton("🚫 Клиент отсутствует", callback_data=f"client_not_home_{order_id}")]
        ]
    ...
    # Уведомление магазина (УПРОЩЕНО)
    try:
        # Используем имя курьера
...
async def cancel_order(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Возвратный заказ отменить нельзя, только обычный
    if o.get("return_for") is not None:
        await q.edit_message_text("⛔ Возвратный заказ не может быть отменен курьером. Свяжитесь с магазином.")
        return
...
async def finish_order(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверка на дублирование с finish_return_order (хотя порядок регистрации должен помочь)
    if q.data.split("_")[1] == 'return':
        return
    ...
    if not o or o["courier_tg_id"] != courier_id or o["status"] != 'at_client':
        await q.answer("⛔ Заказ неактивен или не на финальной стадии.")
        return
...
# Обработка завершения возврата (Возвращен в магазин)
async def finish_return_order(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    # parts: ['finish', 'return', 'order_id']
    try:
        order_id = int(q.data.split("_")[2]) 
    ...
    if not o or o["courier_tg_id"] != courier_id:
        await q.answer("⛔ Заказ неактивен или не принадлежит вам.")
        return
        
    # Проверяем, что это действительно возвратный заказ
    if o["return_for"] is None:
        await q.answer("⛔ Это не возвратный заказ.")
        return

    # update status to delivered for return order
    courier_name = await get_username(courier_id, ctx.application)
    await update_order(order_id, status="delivered", log_add=f"Курьер {courier_name} завершил возврат товара в магазин.")
    
    o = await get_order(order_id)
    report = html_report(order_id, o, courier_name, include_log=True)
    
    # Редактируем сообщение у курьера
    try:
        await q.edit_message_text(report, parse_mode="HTML", reply_markup=None)
    except:
        pass
    ...
    try:
        ...
        logger.warning(f"Не удалось отправить отчет магазину о возврате: {e}")

    # Удаляем записи о сообщении из базы
    await delete_courier_message_records(order_id)


# ========== Клиент отсутствует -> УВЕДОМЛЕНИЕ МАГАЗИНА + КНОТКА ВОЗВРАТА ==========
async def client_not_home_entry(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # ИСПРАВЛЕНИЕ: Используем parts[-1] для надежности
        order_id = int(parts[-1]) 
    except (IndexError, ValueError):
        await q.edit_message_text("⛔ Неверные данные в запросе. Операция отменена.")
        return
    
    orig = await get_order(order_id)
    if not orig or orig["courier_tg_id"] != courier_id or orig["status"] != "at_client":
        await q.edit_message_text("⛔ Этот заказ не ваш или не на верной стадии.")
        return
    
    courier_name = await get_username(courier_id, ctx.application)

    # 1. Закрываем оригинальный заказ как failed_delivery (с сохранением полной стоимости)
    await update_order(order_id, status="failed_delivery", log_add=f"Клиент отсутствует. Требуется создание возвратного заказа магазином.")
    orig = await get_order(order_id) # Обновляем данные
    report = html_report(order_id, orig, courier_name, include_log=True)

    # 2. Деактивируем/редактируем старое сообщение о заказе у курьера
    try:
        # Редактируем сообщение курьера и отправляем ему полный отчет
        await q.edit_message_text(report, parse_mode="HTML", reply_markup=None)
        await delete_courier_message_records(order_id)
        
    except Exception as e:
        logger.warning(f"Не удалось деактивировать сообщение курьеру при отсутствии клиента: {e}")
        
    # 3. Уведомление магазина (с кнопкой для создания возврата)
    try:
        ...
        await ctx.application.bot.send_message(
            ...
            f"Курьер {courier_name} ожидает решения. Товар у него. \n\n"
            f"Создайте новый заказ для возврата товара.",
            parse_mode="HTML",
            reply_markup=kb
        )
    except:
...
async def shop_cancel_from_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    courier_param = None
    status = "cancelled"
    paid_status = PAYMENT_STATUS_UNPAID # По умолчанию

    
    if is_on_delivery and courier_id and o["paid_to_courier"] == PAYMENT_STATUS_UNPAID:
        # Курьер уже забрал товар и начал доставку, но не доехал до клиента. Компенсация 70%.
        amount = await pay_courier_fixed_amount(order_id, courier_id, price, "Компенсация 70% за отмену в пути")
        log_message = f"Магазин отменил заказ. Курьеру должна быть выплачена компенсация 70% ({int(amount)} ₽). Товар остается у курьера."
        status = "cancelled_70_percent"
        courier_param = courier_id # Оставляем курьера, чтобы он получил уведомление
        paid_status = PAYMENT_STATUS_UNPAID # Еще не оплачено
        
    elif o["status"] == "taken" or o["status"] == "at_shop":
        # Курьер только взял заказ или едет к магазину/забрал, но магазин отменил. 
        status = "new"
        courier_param = 0
        ...
        # Для других статусов (например, failed_delivery или уже завершен)
        status = "cancelled"
        courier_param = courier_id if courier_id else 0
        paid_status = o["paid_to_courier"] # Сохраняем текущий статус оплаты
    ...
    # Деактивация сообщений у всех курьеров
    try:
        existing = await get_courier_message_records(order_id)
        for cid, mid in existing:
            try:
    ...
    # Если заказ был активным, уведомляем курьера.
    if courier_id:
        try:
    ...
    # Если статус стал 'new', рассылаем заново
    if status == 'new':
        await send_order_to_couriers(order_id, ctx.application)


# ========== FINANCE (shop) - Тотальная очистка ==========
async def finance(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    role = await get_role(tg)
    if role != "shop":
        reply_msg = await update.message.reply_html("Команда доступна только магазину.")
        await register_bot_message(ctx, reply_msg.message_id)
        return
...
# УДАЛЕНО: Эта логика перенесена в shop_mark_paid_cb и courier_confirm_payment_cb
...
async def finance_pay_all_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    shop_id = q.from_user.id
    # find all unpaid orders for this shop that should be paid
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("""
    ...
    await q.edit_message_text("⚠️ В целях безопасности, массовая оплата должна быть подтверждена каждым курьером. Мы инициируем процесс для всех неоплаченных заказов.")
    
    # Мы только меняем статус на "ОПЛАЧЕНО МАГАЗИНОМ", чтобы курьеры могли подтвердить.
    count = 0
    for r in rows:
        oid, courier_id, price, status = r
        if r[3] == PAYMENT_STATUS_UNPAID: # Если статус 0
            await update_order(oid, paid=PAYMENT_STATUS_MARKED_PAID, log_add="Магазин инициировал массовую оплату.")
            count += 1
            # Уведомляем курьера, как в shop_mark_paid_cb
            if courier_id:
                try:
                    kb_courier = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Подтвердить оплату", callback_data=f"courier_confirm_payment_{oid}")]])
                    await ctx.application.bot.send_message(
                        courier_id,
                        f"🔔 <b>ПОСТУПИЛА МАССОВАЯ ОПЛАТА!</b>\nМагазин #{shop_id} отметил заказ #{oid} как оплаченный. Подтвердите получение.",
                        parse_mode="HTML",
                        reply_markup=kb_courier
                    )
                except:
    ...
    await ctx.application.bot.send_message(shop_id, f"Инициирована оплата для {count} заказов. Ожидайте подтверждения курьерами.", parse_mode="HTML")
    

# ========== PAYOUTS (courier) - Тотальная очистка ==========
async def payouts(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    role = await get_role(tg)
    if role != "courier":
        reply_msg = await update.message.reply_html("Команда доступна только курьеру.")
        await register_bot_message(ctx, reply_msg.message_id)
        return
...
async def payouts_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "payouts": # Назад к меню
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💰 Полученные выплаты", callback_data="payouts_received")],
            [InlineKeyboardButton("💸 Должны выплатить", callback_data="payouts_unpaid")],
            [InlineKeyboardButton("📊 Сводка", callback_data="payouts_summary")]
    ...
    if action == "received":
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            cur1 = await db.execute("""
                SELECT SUM(CASE WHEN status='cancelled_70_percent' THEN price * 0.70 ELSE price END) 
                FROM orders WHERE courier_tg_id=? 
                AND status IN ('delivered', 'failed_delivery', 'cancelled_70_percent', 'completed_with_return')
            """, (tg,))
            row1 = await cur1.fetchone() # Всего должно быть выплачено (по завершенным)
            
            cur2 = await db.execute("""
                SELECT SUM(CASE WHEN status='cancelled_70_percent' THEN price * 0.70 ELSE price END) 
                FROM orders WHERE courier_tg_id=? AND paid_to_courier=?
            """, (tg, PAYMENT_STATUS_CONFIRMED))
            row2 = await cur2.fetchone() # Всего оплачено
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(shop_set_return_order_entry, pattern="^create_return_from_fail_"), # Обработчик создания возврата
        ],
        states={
    ...
    # callbacks и команды для выполнения заказа и возврата
    ...
    # Кнопки оплаты (Новая логика)
    app.add_handler(CallbackQueryHandler(shop_mark_paid_cb, pattern="^shop_mark_paid_"))
    app.add_handler(CallbackQueryHandler(courier_confirm_payment_cb, pattern="^courier_confirm_payment_"))
    ...
    # Информационные кнопки (чтобы не выдавать ошибку)
    app.add_handler(CallbackQueryHandler(lambda q, c: q.answer("Заказ в работе."), pattern="^info_return_active$|^info_waiting_return$|^info_confirm_wait$|^info_wait$|^info_success$"))
    ...
    app.add_handler(CommandHandler("unpaid", unpaid_orders)) # НОВАЯ КОМАНДА
