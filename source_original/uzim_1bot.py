 # bot.py — полный файл с проверкой телефона
import time
import aiosqlite
import logging
# ===== OWNER =====
OWNER_ID = 1309289031  # <-- сюда вставь СВОЙ Telegram user_id
...
async def init_db(app):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            # users: tg_id unique, role current role, phone
            await db.execute("""
            ...
            # Миграция для старых БД (добавляем колонку phone, если её нет)
            try:
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👑 Админка (пока заглушка)")
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(shop_set_return_order_entry, pattern="^create_return_from_fail_"),
        ],
        states={
    ...
    # Обработчик контакта - ДОЛЖЕН БЫТЬ ПЕРЕД командами
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel))
