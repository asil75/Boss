...
async def finish_order(update: Update, ctx):
    ...
    try:
        # Редактируем сообщение у курьера (финальный отчет без кнопок)
        await q.edit_message_text(report, parse_mode="HTML", reply_markup=None)
    except:
        pass

    try:
        # Отправляем финальный отчет магазину
        # Добавим кнопку "Написать курьеру" в отчет магазина (если нужно)
        shop_kb = None
        if courier_id:
            shop_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("✉️ Написать курьеру", url=f"tg://user?id={courier_id}")]
    ...
    # деактивируем/удаляем все записи рассылки (включая у других курьеров) — больше не нужны
    try:
        existing = await get_courier_message_records(order_id)
        for cid, mid in existing:
            try:
