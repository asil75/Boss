...
async def show_all_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Используем первый order_id для деталей (нужно переделать для каждого заказа отдельно)
    first_order_id = orders[0][0] if orders else None
    if first_order_id:
        buttons_row.append([InlineKeyboardButton("🔍 Детали", callback_data=f"admin_order_detail_{first_order_id}")])
...
async def broadcast_send_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• Процент успеха: {(sent/total_users*100):.1f}%" if total_users > 0 else "• Процент успеха: 0%",
        parse_mode="HTML"
    )
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        # Для обновления логов нужно переделать логику
        await query.edit_message_text("🔄 Логи обновлены...")
        # Здесь нужно вызвать admin_logs снова, но это сложно из callback
        # Лучше сделать отдельную функцию для обновления логов
...
def register_admin_handlers(app):
    ...
    broadcast_conv = ConversationHandler(
        ...
        per_message=True  # Добавлено для правильной работы с callback
    )
