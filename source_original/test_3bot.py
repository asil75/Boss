...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Если телефон есть — проверяем роль и показываем соответствующее меню
    role = await get_role(tg)
    
    if role in ["shop", "courier"]:
        # Если роль уже выбрана, показываем меню роли
        await show_role_menu(update, ctx)
    else:
...
async def show_role_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_msg = await context.application.bot.send_message(
        user_id,
        text,
        parse_mode="HTML",
...
async def handle_menu_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "shop":
        if text == "📦 Активные заказы":
            await myorders(update, context)
        elif text == "💰 Финансы":
            await finance(update, context)
        elif text == "📊 Статистика":
            await stats(update, context)
        elif text == "📋 История заказов":
            await completed_orders(update, context)
        elif text == "⚙️ Мой профиль":
            await whoami(update, context)
        elif text == "🆘 Помощь":
            await update.message.reply_text(
                "🆘 <b>Помощь для магазина:</b>\n\n"
                "• Для создания заказа нажмите '➕ Новый заказ'\n"
                "• Для просмотра активных заказов - '📦 Активные заказы'\n"
                "• Финансы и оплата - '💰 Финансы'\n"
                "• История всех заказов - '📋 История заказов'\n\n"
        ...
        if text == "📦 Взять заказ":
        ...
        elif text == "🗺️ Мои доставки":
            await myorders(update, context)
        elif text == "💰 Выплаты":
            await payouts(update, context)
        elif text == "📊 Статистика":
            await stats(update, context)
            ...
            await update.message.reply_text(
                "🆘 <b>Помощь для курьера:</b>\n\n"
                "• Для поиска заказов нажмите '📦 Взять заказ'\n"
                "• Ваши активные доставки - '🗺️ Мои доставки'\n"
                "• Выплаты и заработок - '💰 Выплаты'\n"
                "• Ваш рейтинг и отзывы - '⭐ Мой рейтинг'\n\n"
                "По вопросам: @ваш_контакт",
                parse_mode="HTML"
            )
...
async def show_available_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает доступные заказы для курьера"""
    user_id = update.effective_user.id
    
    async with aiosqlite.connect(DB_PATH) as db:
        # Получаем новые заказы
        cursor = await db.execute("""
            SELECT o.id, o.price, o.from_address, o.to_address, 
                   o.client_name, o.created_at
            FROM orders o
            WHERE o.status = 'new'
    ...
    if not orders:
        await update.message.reply_text(
            ...
            "Попробуйте позже или проверьте '🗺️ Мои доставки'."
        )
        return
    ...
    for order in orders:
        order_id, price, from_addr, to_addr, client_name, created_at = order
        
        # Форматируем время
        created_time = datetime.fromtimestamp(int(created_at)).strftime('%H:%M')
        ...
        text += (
            ...
            f"📍 От: {from_addr[:30]}...\n"
            f"🎯 До: {to_addr[:30]}...\n"
            f"👤 {client_name or 'Клиент'}\n"
            f"🕒 {created_time}\n"
            f"[<code>take_{order_id}</code>] - взять заказ\n"
            f"{'─' * 30}\n"
        )
    
    keyboard = [
        [InlineKeyboardButton("🔄 Обновить список", callback_data="refresh_orders")],
...
async def show_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cursor = await db.execute("""
            SELECT 
                ...
                AVG(CASE WHEN status = 'delivered' THEN price ELSE 0 END) as avg_earning
            FROM orders 
            WHERE courier_tg_id = ?
        """, (user_id,))
        stats = await cursor.fetchone()
    
    total, completed, failed, avg_earning = stats
    
    # Рассчитываем рейтинг
    if total > 0:
        success_rate = (completed / total) * 100
        rating = min(5.0, 3.0 + (success_rate / 25))  # От 3 до 5 звёзд
    else:
        success_rate = 0
        rating = 3.0
        avg_earning = 0
    ...
    text = (
        f"⭐ <b>Ваш рейтинг: {rating:.1f}/5.0</b>\n\n"
        f"📊 <b>Статистика:</b>\n"
        f"• Всего заказов: {total or 0}\n"
        f"• Успешных: {completed or 0}\n"
        f"• Неудачных: {failed or 0}\n"
        f"• Процент успеха: {success_rate:.1f}%\n"
        f"• Средний заработок: {int(avg_earning or 0)}₽\n\n"
        f"<i>Рейтинг влияет на приоритет получения заказов</i>"
    )
...
async def set_role_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, role: str):
    """Установка роли через кнопку меню"""
    user_id = update.effective_user.id
    
    # Проверяем, есть ли активные заказы у курьера
    if role == "courier":
        async with aiosqlite.connect(DB_PATH) as db:
            cur = await db.execute("SELECT COUNT(*) FROM orders WHERE courier_tg_id=? AND status='taken'", (user_id,))
            row = await cur.fetchone()
            await cur.close()
        
        if row and row[0] > 0:
            await update.message.reply_text(
                "⚠️ <b>У вас есть активные доставки!</b>\n"
                "Сначала завершите их или отмените, чтобы сменить роль.",
                parse_mode="HTML"
            )
            return
...
async def role_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "role_shop":
        await set_role_callback(update, ctx, "shop")
    else:
        await set_role_callback(update, ctx, "courier")
...
# ========== NEW / EDIT ORDER (FSM) - ИСПРАВЛЕННАЯ ВЕРСИЯ ==========
async def new_order_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    tg = update.effective_user.id
    if await get_role(tg) != "shop":
        # Отправляем сообщение и регистрируем его
        reply_msg = await update.message.reply_html("Только магазин может создавать заказы")
        await register_bot_message(ctx, reply_msg.message_id)
        return ConversationHandler.END
    
    # ВАЖНО: Не очищаем весь контекст при запуске из меню!
    ...
    # Очищаем старые сообщения бота, но не трогаем user_data полностью
    await purge_chat_history(ctx.application, tg, ctx)
    
    # Удаляем сообщение команды (если есть)
    if update.message:
        try:
            await ctx.application.bot.delete_message(
    ...
    # Отправляем первый шаг и регистрируем сообщение
    reply_msg = await ctx.application.bot.send_message(
        tg,
        "📝 <b>Шаг 1/6 — Адрес магазина</b>\nВведите адрес отправки:",
        parse_mode="HTML"
    )
    await register_bot_message(ctx, reply_msg.message_id)
    return ADDRESS_FROM
...
async def edit_order_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    reply_msg = await ctx.application.bot.send_message(
        tg,
        f"✍️ <b>Редактирование заказ #{order_id}</b>\n"
...
# ================== MAIN ==================
def main():
    # Создаем приложение ПЕРВЫМ делом
    ...
    # 1. Глобальная проверка блокировки
    app.add_handler(TypeHandler(Update, global_block_guard), group=-1)

    # 2. FSM для создания заказа (ИСПРАВЛЕННАЯ ВЕРСИЯ)
    conv = ConversationHandler(
        entry_points=[
            CommandHandler("new_order", new_order_start),
            MessageHandler(filters.Regex("^➕ Новый заказ$"), new_order_start),  # ДОБАВЛЕНО
    ...
    # НОВЫЕ callback для меню
    app.add_handler(CallbackQueryHandler(
        lambda q, c: show_available_orders(q.message, c), 
        pattern="^refresh_orders$"
    ))
    app.add_handler(CallbackQueryHandler(
        lambda q, c: q.answer("Карта в разработке ⏳"),
        pattern="^show_map$"
    ))
    app.add_handler(CallbackQueryHandler(
        lambda q, c: show_role_menu(q.message, c),
    ...
    # ========== НОВЫЕ ОБРАБОТЧИКИ ДЛЯ МЕНЮ ==========
    # УДАЛЕНА СТРОКА: app.add_handler(MessageHandler(filters.Regex("^➕ Новый заказ$"), new_order_start))
    # Теперь эта кнопка обрабатывается через ConversationHandler
    
    app.add_handler(MessageHandler(filters.Regex("^📦 Активные заказы$"), myorders))
    ...
    app.add_handler(MessageHandler(filters.Regex("^📋 История заказов$"), completed_orders))
    app.add_handler(MessageHandler(filters.Regex("^⚙️ Мой профиль$"), whoami))
    app.add_handler(MessageHandler(filters.Regex("^🆘 Помощь$"), handle_menu_button))
    
    app.add_handler(MessageHandler(filters.Regex("^📦 Взять заказ$"), show_available_orders))
    app.add_handler(MessageHandler(filters.Regex("^🗺️ Мои доставки$"), myorders))
    ...
    app.add_handler(MessageHandler(filters.Regex("^🏪 Я магазин$"), lambda u,c: set_role_callback(u, c, "shop")))
    app.add_handler(MessageHandler(filters.Regex("^🛵 Я курьер$"), lambda u,c: set_role_callback(u, c, "courier")))
    app.add_handler(MessageHandler(filters.Regex("^❌ Отмена$"), show_role_menu))
    
    # Обработчик кнопок меню (общий)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_menu_button))

    # 5. Подключаем плагины ПОСЛЕ создания app
    try:
    ...
    try:
        ...
        logger.info("Admin panel plugin loaded")
    except Exception as e:
        logger.warning(f"Admin panel plugin NOT loaded: {e}")
