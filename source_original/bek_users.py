...
async def toggle_block_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            logger.info(f"Блокировка пользователя {tg_id}: текущий={current_status}, новый={new_status}")
            ...
            if check_row and check_row[0] == new_status:
                ...
                logger.error(f"Не удалось обновить статус пользователя {tg_id}. Ожидалось: {new_status}, получили: {check_row[0] if check_row else 'None'}")
                await query.answer("❌ Ошибка при обновлении статуса", show_alert=True)
                
    except Exception as e:
        logger.error(f"Ошибка при блокировке пользователя {tg_id}: {e}")
